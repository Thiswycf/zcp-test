...
这里的第一组 Spearman 是“代理与 NB201 真值”的方向调整后相关性；最后一列是同一代理在不同
初始化/输入 seed 下的逐架构 score 排名一致性。二者回答不同问题，不能互相替代。`params` 与
`flops` 注册为 `minimize`，因此报告在计算相关性前执行 `negated`；若研究原始规模—精度关联，
必须另行报告未反向结果。
