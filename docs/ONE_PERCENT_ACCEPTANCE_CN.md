...
自 2026-08-04 起，未来训练“双重 1%”工程验收只训练一个 ZCP-selected 架构的两项协议，不再将固定
随机和参数量/FLOPs 匹配候选纳入 acceptance。历史三候选结果仍保留。原因是短训适合验证工程链路，
不具备证明搜索收益的充分统计与收敛条件；候选优劣应在独立、充分训练、多 seed 协议中研究。
