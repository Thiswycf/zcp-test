...
if (data && Array.isArray(data.tasks) && Array.isArray(data.risks) && Array.isArray(data.evidence)) {
  ...
  const dartsResumeStatus = "恢复闭环已完成，六项验收待启动";
  assert(dartsTask?.status === dartsResumeStatus, "DARTS 任务状态未反映恢复闭环和下一阶段边界");
  assert(highCostTask?.status === dartsResumeStatus, "DARTS ImageNet 高成本验收状态不准确");
  assert(dartsLiveTask?.status === dartsResumeStatus, "DARTS ImageNet 实时任务状态不准确");
  ...
  assert(dartsNextSix?.result.includes("不外推为已启动、已完成或通过"), "DARTS 六项验收准备状态边界缺失");
  const fullGate448 = data.evidence.find((entry) => entry.id === "EV-FULL-GATE-448-COLLECT");
  assert(fullGate448?.result.includes("448 tests") && fullGate448?.result.includes("collect 数"), "最新全仓 448 collect 计数缺失");
  assert(fullGate448?.result.includes("source coverage 87%") && fullGate448?.result.includes("CLI coverage 82%"), "最新全仓 coverage 缺失");
  assert(fullGate448?.command === "<audit-root>/integration-full-gate-post-autoformer-20260731.log", "最新全仓门禁脱敏日志路径不准确");
