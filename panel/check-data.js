...
if (data && Array.isArray(data.tasks) && Array.isArray(data.risks) && Array.isArray(data.evidence)) {
  ...
  assert(parallelRun?.result.includes("parallel workflow") && parallelRun?.result.includes("仍为 running"), "单卡 parallel workflow 身份或状态缺失");
  ...
  assert(parallelRun?.result.includes("params-matched 1%-data 当前 137/250") && parallelRun?.result.includes("总体 5/6 完成") && parallelRun?.result.includes("workflow 仍为 running") && parallelRun?.result.includes("不得标六项 completed"), "DARTS 5/6 运行边界缺失");
  ...
  assert(dartsLiveTask?.detail.includes("parallel workflow 仍 running"), "DARTS 实时任务未同步 parallel workflow");
  assert(dartsLiveTask?.detail.includes("fixed-random、params-matched 三项 full-data 均 3/3") && dartsLiveTask?.detail.includes("38.624 与 29.852") && dartsLiveTask?.detail.includes("10.6 与 9.6"), "DARTS 实时任务未同步五项完成指标");
  assert(dartsLiveTask?.detail.includes("params-matched 1%-data 为 137/250") && dartsLiveTask?.detail.includes("workflow 仍 running"), "DARTS 实时任务未同步最后一项运行状态");
  ...
  assert(dartsRisk?.status === "开放" && dartsRisk?.description.includes("总体 5/6") && dartsRisk?.description.includes("params-matched 1%-data 为 137/250") && dartsRisk?.description.includes("仍为 running"), "DARTS 六项风险状态不准确");
  ...
  assert(longScheduleTask?.detail.includes("250/250") && longScheduleTask?.detail.includes("10.6") && longScheduleTask?.detail.includes("9.6") && longScheduleTask?.detail.includes("137/250") && longScheduleTask?.detail.includes("5/6"), "H3 未同步 DARTS 1%-data 最新状态");
