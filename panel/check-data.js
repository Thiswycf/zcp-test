...
if (data && Array.isArray(data.tasks) && Array.isArray(data.risks) && Array.isArray(data.evidence)) {
  ...
  assert(parallelRun?.result.includes("commit 04dac24") && parallelRun?.result.includes("四条单卡 lane") && parallelRun?.result.includes("北京时间 2026-07-31 14:49") && parallelRun?.result.includes("status=running"), "单卡 parallel workflow 身份或状态缺失");
  assert(parallelRun?.result.includes("各约 8.5 batch/s × 128") && parallelRun?.result.includes("合计约 2176 img/s") && parallelRun?.result.includes("旧四卡 DDP 约 9.6 batch/s × 128") && parallelRun?.result.includes("约 1229 img/s"), "新旧 workflow 吞吐实测缺失");
  assert(parallelRun?.result.includes("1% schedule 至约 epoch 7") && parallelRun?.result.includes("4.8–5.5 GiB/卡") && parallelRun?.result.includes("170–210 W"), "parallel workflow 进度或资源实测缺失");
  ...
  assert(dartsLiveTask?.detail.includes("commit 04dac24") && dartsLiveTask?.detail.includes("tasks 2–5 并发") && dartsLiveTask?.detail.includes("status=running"), "DARTS 实时任务未同步 parallel workflow");
  assert(dartsLiveTask?.detail.includes("合计约 2176 img/s") && dartsLiveTask?.detail.includes("1% schedule 已运行至约 epoch 7") && dartsLiveTask?.detail.includes("8769 根目录"), "DARTS 实时任务未同步吞吐、进度或 monitor");
  ...
  assert(dartsRisk?.status === "开放" && dartsRisk?.description.includes("旧 task2 已 interrupted") && dartsRisk?.description.includes("tasks 2–5 并发") && dartsRisk?.description.includes("六项总体仍未完成"), "DARTS 六项风险状态不准确");
