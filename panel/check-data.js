...
if (data && Array.isArray(data.tasks) && Array.isArray(data.risks) && Array.isArray(data.evidence)) {
  ...
  assert(parallelRun?.result.includes("fixed-random 已完成 2/3") && parallelRun?.result.includes("valid top1=33.916") && parallelRun?.result.includes("train 1041.06 samples/s"), "full-data fixed-random 最新指标缺失");
  assert(parallelRun?.result.includes("params-matched 已完成 2/3") && parallelRun?.result.includes("valid top1=22.058") && parallelRun?.result.includes("train 1063.96 samples/s"), "full-data params-matched 最新指标缺失");
  assert(parallelRun?.result.includes("215/250") && parallelRun?.result.includes("213/250"), "one-percent schedule 最新里程碑缺失");
  assert(parallelRun?.result.includes("affinity 最终为 0–127") && parallelRun?.result.includes("六项仍未完成") && parallelRun?.result.includes("不得标 completed"), "最终 affinity 或未完成边界缺失");
  ...
  assert(dartsLiveTask?.detail.includes("parallel workflow 仍为 running"), "DARTS 实时任务未同步 parallel workflow");
  assert(dartsLiveTask?.detail.includes("valid top1 分别为 33.916 与 22.058") && dartsLiveTask?.detail.includes("1041.06 与 1063.96 samples/s") && dartsLiveTask?.detail.includes("215/250") && dartsLiveTask?.detail.includes("213/250"), "DARTS 实时任务未同步训练里程碑");
  assert(dartsLiveTask?.detail.includes("affinity 最终为 0–127") && dartsLiveTask?.detail.includes("8769 monitor 根目录") && dartsLiveTask?.detail.includes("HTTP 服务重启后验证可访问"), "DARTS 实时任务未同步 affinity 或 monitor 恢复");
  ...
  assert(dartsRisk?.status === "开放" && dartsRisk?.description.includes("均为 2/3") && dartsRisk?.description.includes("215/250") && dartsRisk?.description.includes("213/250") && dartsRisk?.description.includes("六项总体尚未完成"), "DARTS 六项风险状态不准确");
  ...
  assert(dartsTask?.detail.includes("affinity 最终保持 0–127") && dartsTask?.detail.includes("ZCP_CPU_AFFINITIES") && dartsTask?.detail.includes("不设默认"), "D1 未同步 affinity 最终状态");
  assert(dartsLiveTask?.detail.includes("affinity 最终为 0–127"), "J4 未同步 affinity 最终状态");
  assert(dartsRisk?.description.includes("affinity 最终保持 0–127"), "DARTS 风险未同步 affinity 最终状态");
  ...
  assert(highCostTask?.detail.includes("valid top1=33.916") && highCostTask?.detail.includes("1041.06 samples/s") && highCostTask?.detail.includes("valid top1=22.058") && highCostTask?.detail.includes("1063.96 samples/s"), "H2 未同步 full-data epoch 1 指标");
  assert(longScheduleTask?.detail.includes("215/250") && longScheduleTask?.detail.includes("213/250") && longScheduleTask?.detail.includes("不能标 completed"), "H3 未同步 one-percent 里程碑或完成边界");
