...
class EvolutionSearch:
    ...
    def _restore(self, state: Mapping[str, Any]) -> None:
        ...
        if any(
            candidate.architecture.architecture_id not in restored_cache
            or restored_cache[candidate.architecture.architecture_id] != candidate.score
    ...
    def _record(self, generation: int, candidate: Candidate, cache_hit: bool, selected: bool) -> None:
        self.writer.append({**self.record_metadata, "record_kind": "candidate", "generation": generation, "search_space_id": candidate.architecture.search_space_id, "architecture_id": candidate.architecture.architecture_id, "architecture": candidate.architecture.spec, "parents": candidate.parents, "operation": candidate.operation, "score": candidate.score, "cache_hit": cache_hit, "selected": selected, "cumulative_evaluations": self.evaluations, "cumulative_cache_hits": self.cache_hits, "elapsed_seconds": self._elapsed()})
